...
function loadState(){
  try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY));if(saved&&Array.isArray(saved.workers)&&Array.isArray(saved.entries)&&Array.isArray(saved.withdrawals))return saved}catch(e){}
  ...
  try{
    ...
    if(old&&Array.isArray(old.workers)&&Array.isArray(old.entries)){
      ...
      fresh.entries=old.entries.map(e=>({id:e.id,date:e.date,doctorId:e.doctorId,consultations:number(e.consultations),generated:number(e.generated),notes:e.notes||''}));
...
function render(){
  ...
  renderDoctorOptions();renderSummary();renderEntries();renderWithdrawals();renderDoctorStats();renderWorkersCount();
}
...
function renderSummary(){
  ...
  const generated=entries.reduce((s,e)=>s+number(e.generated),0);
  const taken=withdrawals.reduce((s,w)=>s+number(w.amount),0);
  $('summaryConsultations').textContent=fmt(consultations);$('summaryGenerated').textContent=fmt(generated);$('summaryTaken').textContent=fmt(taken);$('summaryBalance').textContent=fmt(generated-taken);
}
...
function renderDoctorStats(){
  ...
  const totals=workers.map(w=>{const own=entries.filter(e=>e.doctorId===w.id),ownW=withdrawals.filter(x=>x.doctorId===w.id);return{w,consultations:own.reduce((s,e)=>s+number(e.consultations),0),generated:own.reduce((s,e)=>s+number(e.generated),0),taken:ownW.reduce((s,x)=>s+number(x.amount),0)}});
...
function submitEntry(e){
  ...
  const data={date,doctorId,consultations,generated:number($('entryGenerated').value),notes:$('entryNotes').value.trim()};
...
function openWithdrawalDialog(){if(!activeWorkers().length){showToast('Ajoutez d’abord un médecin');openWorkers();return}resetWithdrawalForm();$('withdrawalDialog').showModal()}
...
function submitWithdrawal(e){
  e.preventDefault();const doctorId=$('withdrawalDoctor').value,date=$('withdrawalDate').value,amount=number($('withdrawalAmount').value);if(!doctorId||!date||amount<=0)return;
  const data={date,doctorId,amount,notes:$('withdrawalNotes').value.trim()};
  if(editingWithdrawalId){const i=state.withdrawals.findIndex(x=>String(x.id)===String(editingWithdrawalId));if(i>=0)state.withdrawals[i]={...state.withdrawals[i],...data};showToast('Retrait modifié')}else{state.withdrawals.push({id:`r${Date.now()}`,...data});showToast('Retrait enregistré')}
  currentMonth=date.slice(0,7);saveState();render();closeWithdrawalDialog();
}
function editWithdrawal(id){const w=state.withdrawals.find(x=>String(x.id)===String(id));if(!w)return;editingWithdrawalId=w.id;renderDoctorOptions();$('withdrawalDate').value=w.date;$('withdrawalDoctor').value=w.doctorId;$('withdrawalAmount').value=w.amount;$('withdrawalNotes').value=w.notes||'';$('withdrawalDialogTitle').textContent='Modifier le retrait';$('withdrawalDialog').showModal()}
function deleteWithdrawal(id){if(!confirm('Supprimer ce retrait ?'))return;state.withdrawals=state.withdrawals.filter(w=>String(w.id)!==String(id));saveState();render();showToast('Retrait supprimé')}
...
function exportData(){
  ...
  const shifts=[...state.entries].sort((a,b)=>a.date.localeCompare(b.date)).map(e=>({Date:excelDate(e.date),Travailleur:doctorById(e.doctorId)?.name||'Médecin',Consultations:number(e.consultations),'Total généré (MRU)':number(e.generated),Remarque:e.notes||''}));
  const totalConsultations=shifts.reduce((s,r)=>s+r.Consultations,0),totalGenerated=shifts.reduce((s,r)=>s+r['Total généré (MRU)'],0);
  shifts.push({Date:'',Travailleur:'TOTAL',Consultations:totalConsultations,'Total généré (MRU)':totalGenerated,Remarque:''});
  ...
  const resume=[{Indicateur:'Total consultations',Valeur:totalConsultations},{Indicateur:'Total généré (MRU)',Valeur:totalGenerated},{Indicateur:'Total retiré (MRU)',Valeur:totalWithdrawn},{Indicateur:'Solde (MRU)',Valeur:totalGenerated-totalWithdrawn}];
  const wb=XLSX.utils.book_new();
  const ws1=XLSX.utils.json_to_sheet(shifts,{header:['Date','Travailleur','Consultations','Total généré (MRU)','Remarque']});ws1['!cols']=[{wch:13},{wch:24},{wch:15},{wch:20},{wch:42}];ws1['!autofilter']={ref:`A1:E${Math.max(2,shifts.length)}`};
  ...
  const ws3=XLSX.utils.json_to_sheet(resume);ws3['!cols']=[{wch:26},{wch:18}];
...
async function importData(file){if(!file)return;try{const data=JSON.parse(await file.text());if(!Array.isArray(data.workers)||!Array.isArray(data.entries))throw new Error();state={workers:data.workers,entries:data.entries,withdrawals:Array.isArray(data.withdrawals)?data.withdrawals:[]};saveState();render();showToast('Sauvegarde importée')}catch(e){alert('Impossible d’importer cette sauvegarde.')}$('importInput').value=''}
